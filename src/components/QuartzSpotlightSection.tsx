...
const CARDS = [
  {
    catalogHref: "/catalog/quartz" as const,
    image: coverA,
    title: "Quartz",
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto grid w-full max-w-[min(95vw,74rem)] grid-cols-1 gap-14 lg:grid-cols-2 lg:gap-[4.6rem]">
        {CARDS.map((card) => {
          const outerHoverGroup = card.catalogHref ? "" : "group ";
          const article = (
            <article
              className={`${outerHoverGroup}relative flex min-h-[clamp(35rem,68vh,46rem)] flex-col overflow-hidden rounded-[0.4rem] border border-white/[0.1] bg-[#2f3137]/92 p-6 shadow-[0_20px_50px_rgba(0,0,0,0.42)] sm:min-h-[clamp(37rem,71vh,49rem)] sm:p-7`}
            >
              ...
              <div className="relative z-10 mt-auto flex items-center justify-between border-t border-white/[0.08] pt-6">
                <p className="font-sans text-[1.42rem] font-semibold uppercase leading-[1.12] tracking-[0.06em] text-white/92 sm:text-[1.6rem]">{card.title}</p>
                <span
          ...
          return <div key={card.title}>{article}</div>;
        })}
      </div>
    </section>
  );
