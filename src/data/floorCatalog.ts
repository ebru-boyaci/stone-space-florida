...
const floorModules = import.meta.glob("../../assets/floor/*.{avif,jpg,jpeg,png,webp}", {
  eager: true,
...
function floorSlugNumericOrder(slug: string): number {
  const m = slug.match(/^floor-(\d+)$/i);
  return m ? parseInt(m[1], 10) : Number.MAX_SAFE_INTEGER;
}

/**
 * Zemin swatch’ları — `assets/floor/` (glob). Örnek: `{slug}-example.png`, çoklu: `{slug}-example-1.png`, …
 */
export function getFloorCatalogItems(): CatalogGalleryItem[] {
  const moduleByBase = new Map<string, string>();
  for (const [path, mod] of Object.entries(floorModules)) {
  ...
  return [...covers.entries()]
    ...
    .sort((a, b) => {
      const na = floorSlugNumericOrder(a.slug);
      const nb = floorSlugNumericOrder(b.slug);
